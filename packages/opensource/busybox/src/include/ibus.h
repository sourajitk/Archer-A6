...
typedef struct _IBUS_ENEVT_DATA_SMARTIP
{
	...
	union {
		U16 detect_result; /*dhcpc detects dhcps result:0 success, 1 fail*/
		U16 wifi_status;	  /*wifi_status:0 no change, 1 connected->disconnected, 2 disconnected->connected, 3 rootap changed*/
		U16 lanip_changed; /*for smartip pub msg:0 lanip no change, 1 lanip changed*/
	} status;
