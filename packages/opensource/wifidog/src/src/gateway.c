...
main_loop(void)
{
	...
	if (!config->gw_id) {
		...
		debug(LOG_ERR, "%s = %s", config->gw_interface, config->gw_id);
	}
	...
	httpdAddFileContent(webserver, "/", "portal.jpg", 0, NULL, DEFAULT_USERPORTALBGRJPG);
	#endif
	httpdAddFileContent(webserver, "/", "portal-icon.png", 0, NULL, DEFAULT_PORTALICONPNG);
...
int main(int argc, char **argv) {
	...
	if (restart_orig_pid) {
		...
		debug(LOG_ERR, "Parent PID %d seems to be dead. Continuing loading.");
	}

	if (config->daemon) {

		debug(LOG_ERR, "Forking into background");
