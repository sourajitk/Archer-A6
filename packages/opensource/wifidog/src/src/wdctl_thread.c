...
thread_wdctl_handler(void *arg)
{
	...
	printf("request the command of wdctl \n");
...
wdctl_restart(int afd)
{
	...
	debug(LOG_ERR, "Will restart myself");
	...
	debug(LOG_ERR, "Socket name: %s", sock_name);
	 ...
	 */
	debug(LOG_ERR, "Forking in preparation for exec()...");
	pid = safe_fork();
	if (pid > 0) {
		/* Parent */
