...
iptables_fw_init(void)
{
	 ...
	 *
	 */
	debug(LOG_ERR, "config->gw_interface is %s",config->gw_interface);
	/* Create new chains */
	...
	iptables_do_command("-t mangle -A PREROUTING -m mark --mark 0x%u -j " TABLE_WIFIDOG_TRUSTED, FW_MAKR_INTERFACE);//this rule will be inserted before the prior one
	iptables_do_command("-t mangle -A PREROUTING -m mark --mark 0x%u -j " TABLE_WIFIDOG_OUTGOING, FW_MAKR_INTERFACE);
	iptables_do_command("-t mangle -A POSTROUTING -m mark --mark 0x%u -j " TABLE_WIFIDOG_INCOMING, FW_MAKR_INTERFACE);
	...
	/* Assign links and rules to these new chains */
    iptables_do_command("-t nat -A PREROUTING -m mark --mark 0x%u -j " TABLE_WIFIDOG_OUTGOING, FW_MAKR_INTERFACE);
    ...
    iptables_do_command("-t filter -I FORWARD -m mark --mark 0x%u -j " TABLE_WIFIDOG_WIFI_TO_INTERNET, FW_MAKR_INTERFACE);
