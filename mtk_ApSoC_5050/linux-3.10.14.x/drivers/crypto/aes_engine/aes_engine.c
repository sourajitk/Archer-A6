...
static int aes_engine_proc_open(struct inode *inode, struct file *file);
static ssize_t aes_engine_proc_write(struct file *file, const char __user *buffer, 
				  size_t count, loff_t *data);
static int aes_engine_proc_read(struct seq_file *seq, void *v);
				  
static const struct file_operations aes_engine_proc_fops = {
	.owner 		= THIS_MODULE,
	.open	 	= aes_engine_proc_open,
	.read	 	= seq_read,
	.write	= aes_engine_proc_write,
	.llseek	 	= seq_lseek,
...
static DECLARE_TASKLET(AesRxTasklet, AesRxHandler, 0);
	...
	)
{
	...
	while(1)
	{
		...
		{	
			...
			if (bUserData == 0)
			{	
				rxdesc->SDP0 = dma_map_single(NULL, Dst0, next_dst->length, DMA_FROM_DEVICE);
  ...
  if (bUserData == 0)
	...
	else
	{
		...
		while (remain_SDL > 0)
		{
			...
			Dst1 = kmalloc(SDL, GFP_KERNEL|GFP_ATOMIC);
			...
			{	
				...
				rxdesc->SDP0 = dma_map_single(NULL, Dst1, SDL, DMA_FROM_DEVICE);
...
void AesRxHandler(unsigned long data)
{
	...
	do
	{
		...
		if (bUserData)
		{
			...
			if (AES_userdata[k].new_SDP0)
			{
				...
				do {
					...
					dma_unmap_single (NULL, AES_Entry.AES_rx_ring0[first_rx].SDP0, AES_Entry.AES_rx_ring0[first_rx].aes_rxd_info2.SDL0, DMA_FROM_DEVICE);
					pos = 0;
					j = last_rx;//k;
					do{
		...
		{	
			dma_unmap_single (NULL, AES_Entry.AES_rx_ring0[k].SDP0, AES_Entry.AES_rx_ring0[k].aes_rxd_info2.SDL0, DMA_FROM_DEVICE);
		}
...
static int aes_engine_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, aes_engine_proc_read, NULL);
}
...
static int __init AesEngineInit(void)
{
	...
	printk("Enable MTK AesEngine Module (verson=%08X) \n", sysRegRead(AES_INFO));
		...
		entry = proc_create(PROCNAME, 0666, pProcDir, &aes_engine_proc_fops);
    if (entry == NULL)
    {
