...
static void esw_link_status_changed(int port_no, void *dev_id)
{
	...
	if(port_no==0)
	{
		netif_carrier_on(__dev_get_by_name(&init_net, "eth1"));
	}
	/* add end */
	...
	if(port_no==0)
	{
		netif_carrier_off(__dev_get_by_name(&init_net, "eth1"));
	}
	/* add end */
    }
}
...
#define PHY_READ_TIME 5
void phy_read_schedule(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	if(dwork == NULL)
	{
		printk(KERN_EMERG "dwork NULL!\n");
		return;
	}
	END_DEVICE *ei_local = container_of(dwork, END_DEVICE, state_queue);
	if (ei_local == NULL)
	{
		printk(KERN_EMERG "ei_local NULL!\n");
		return;
	}
		...
		if((reg_val & 0x1)) 
		{
			...
			if(!netif_carrier_ok(wan_dev))
			{
				if(countA >= 2)
				{
					...
					printk("phy schedule eth1 carrier on\n");
				}
				printk("phy schedule try eth1 carrier on reg=%x\n", reg_val);
			}
		}
		else
		{
			...
			if(netif_carrier_ok(wan_dev))
			{
				if(countB >= 2)
				{
					...
					printk("phy schedule eth1 carrier off\n");
				}
				printk("phy schedule try eth1 carrier off reg=%x\n", reg_val);
			}
		}
		dev_put(wan_dev);
	}

	schedule_delayed_work(&ei_local->state_queue, PHY_READ_TIME * HZ);
}
...
#define RT2860_NVRAM	0
	...
	*((volatile u32 *)(RALINK_SYSCTL_BASE+0x34)) = 0x00000000;
	...
	INIT_DELAYED_WORK(&ei_local->state_queue, phy_read_schedule);
...
int ei_close(struct net_device *dev)
{
	...
	cancel_delayed_work_sync(&ei_local->state_queue);
	printk("cancel phy_read_schedule\n");
...
#else
	/* add by wanghao */
	printk("set TPlink LAN/WAN Partition\n");
	
	mii_mgr_write(31, 0x2004, 0x20ff0003);//port0
	mii_mgr_write(31, 0x2104, 0xff0003);//port1
	mii_mgr_write(31, 0x2204, 0xff0003);//port2
	mii_mgr_write(31, 0x2304, 0xff0003);//port3
	mii_mgr_write(31, 0x2404, 0xff0003);//port4
	mii_mgr_write(31, 0x2504, 0x20ff0003);//port5
	mii_mgr_write(31, 0x2604, 0x20ff0003);//port6
	mii_mgr_write(31, 0x2704, 0x20ff0003);//port7

	mii_mgr_write(31, 0x2010, 0x81000000);//port0
	mii_mgr_write(31, 0x2110, 0x810000c0);//port1
	mii_mgr_write(31, 0x2210, 0x810000c0);//port2
	mii_mgr_write(31, 0x2310, 0x810000c0);//port3
	mii_mgr_write(31, 0x2410, 0x810000c0);//port4
	mii_mgr_write(31, 0x2510, 0x81000000);//port5
	mii_mgr_write(31, 0x2610, 0x81000000);//port6
	mii_mgr_write(31, 0x2710, 0x81000000);//port7

	mii_mgr_write(31, 0x2014, 0x10001);//port0
	mii_mgr_write(31, 0x2114, 0x10002);//port1
	mii_mgr_write(31, 0x2214, 0x10002);//port2
	mii_mgr_write(31, 0x2314, 0x10002);//port3
	mii_mgr_write(31, 0x2414, 0x10002);//port4
	IsSwitchVlanTableBusy();
	
	mii_mgr_write(31, 0x94, 0x50210001);//VAWD1 0010,0001
	mii_mgr_write(31, 0x98, 0x2800);//VAWD2
	mii_mgr_write(31, 0x90, 0x80001001);//VTCR, VID=1
	IsSwitchVlanTableBusy();
	mii_mgr_write(31, 0x94, 0x50de0001);//VAWD1 1101,1110
	mii_mgr_write(31, 0x98, 0x2800);//VAWD2
	mii_mgr_write(31, 0x90, 0x80001002);//VTCR, VID=2
	IsSwitchVlanTableBusy();
	/* add end*/
...
void setup_internal_gsw(void)
{
	...
	*(volatile u_long *)(ETH_GPIO_BASE+0xed0) = 0;     //set GE2 TUNE
	...
	if(phy_auto_up_flag)
	{
		printk("turn on phy in gsw ,flag = %d\n", phy_auto_up_flag);
		for(i=0;i<=4;i++)
		{
			/*turn on PHY*/
			mii_mgr_read(i, 0x0 ,&regValue);
		...
		printk("not turn on phy in gsw ,flag = %d\n", phy_auto_up_flag);
