...
static struct proc_dir_entry *procPhyAutoUp;
...
static int PhyAutoUpRead(struct seq_file *seq, void *v)
{
	seq_printf(seq, "%d\n", phy_auto_up_flag);
	return 0;
}

static int PhyAutoUpOpen(struct inode *inode, struct file *file)
{
	return single_open(file, PhyAutoUpRead, NULL);
}


static ssize_t PhyAutoUpWrite(struct file *file, const char __user *buffer, 
		      size_t count, loff_t *data)
{
	char buf[32];	
	int flag = 0;
	...
	if(ret == 0)
	{
		printk("PhyAutoUpWrite error\n");
		return -1;
	}
	phy_auto_up_flag = flag;
    return count;
}


static const struct file_operations phy_auto_up_fops = {
	.owner 		= THIS_MODULE,
	.open       	= PhyAutoUpOpen,
	.read	 	= seq_read,
	.write 		= PhyAutoUpWrite
};
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,36)
		if ((procPhyAutoUp = create_proc_entry(PROCPHY_AUTO_UP, 0, procRegDir)))
			procPhyAutoUp->proc_fops = &phy_auto_up_fops;
		else
#else
		if (!(procPhyAutoUp = proc_create(PROCPHY_AUTO_UP, 0, procRegDir, &phy_auto_up_fops)))
#endif
			printk("!! FAIL to create %s PROC !!\n", PROCPHY_AUTO_UP);
...
void debug_proc_exit(void)
{
	...
	if (procPhyAutoUp)
		remove_proc_entry(PROCPHY_AUTO_UP, procRegDir);
