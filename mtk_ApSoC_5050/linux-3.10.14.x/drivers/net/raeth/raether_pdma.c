...
void skb_debugFy(const struct sk_buff *skb)
{
#define NUM2PRINT 80
...
/* add by wanghao  */
#ifdef CONFIG_SFE_PPP_HEADER
	if ((!sfe_PppoeToWanParsePtr || sfe_PppoeToWanParsePtr(skb))
		&& (!sfe_PptpToWanParsePtr || sfe_PptpToWanParsePtr(skb))
		&& (!sfe_L2tpToWanParsePtr || sfe_L2tpToWanParsePtr(skb))) {
		//not pppoe, pptp, l2tp
	}
#endif
	/* add end  */
