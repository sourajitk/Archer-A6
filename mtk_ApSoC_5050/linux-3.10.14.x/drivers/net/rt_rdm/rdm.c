...

{
    ...
    rdm_class = class_create(THIS_MODULE, RDM_DEVNAME); 

    if(IS_ERR(rdm_class)) {
        printk(KERN_WARNING "Failed to create rdm class\n");
        return 0;
    }

    device_create(rdm_class, NULL, MKDEV(rdm_major, 0), NULL, RDM_DEVNAME);
#endif
