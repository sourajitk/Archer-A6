...
#else
#define FILE_SYSTEM_START_ADDRESS	0x300000
#endif

#endif
#ifdef CONFIG_RT2880_ROOTFS_IN_FLASH
#define NAND_MTD_ROOTFS_PARTITION_NO            3
#endif
...
#endif
#if defined (__KERNEL_NAND__)
/* No use of mtk partitions. By xjb, 2017.11.10 */
#if 1
extern struct mtd_partition_mtk g_pasStatic_Partition[];
int part_num = 0;
#endif
#endif
#ifdef PMT
...
#else
	if (!mtk_nand_status_ready(STA_NFI_FSM_MASK | STA_NAND_BUSY))
		{
		printk(KERN_ERR "error mtk_nand_status_ready");
		return false;
		}
	if (! mtk_nand_RFIFOValidSize(0))
		{
		printk(KERN_ERR "error mtk_nand_RFIFOValidSize");
		return false;
		}
	if (! mtk_nand_WFIFOValidSize(0))
		{
		printk(KERN_ERR "error mtk_nand_WFIFOValidSize");
		return false;
		}
	return true;
#endif
...
static bool mtk_nand_ready_for_read(struct nand_chip *nand, u32 u4RowAddr, u32 u4ColAddr, bool full, u8 * buf)
{
    ...
    if (!mtk_nand_reset())
    {
    	printk(KERN_ERR "nand havn't been reset!\n");
        goto cleanup;
    }
    if (g_bHwEcc)
    {
    ...
    {
    	printk(KERN_ERR "error:mtk_nand_set_command!\n");
        goto cleanup;
    }
    if (!mtk_nand_set_address(col_addr, u4RowAddr, colnob, rownob))
    {
    printk(KERN_ERR "error:mtk_nand_set_address!\n");
        goto cleanup;
    }
...
bool mtk_nand_exec_read_page(struct mtd_info *mtd, u32 u4RowAddr, u32 u4PageSize, u8 * pPageBuf, u8 * pFDMBuf)
{
    ...
    if (mtk_nand_ready_for_read(nand, u4RowAddr, 0, true, buf))
    {
		...
		for (j = 0 ; j < u4SecNum; j++)
		{
			...
			{
				printk("error mtk_nand_read_page_data\n");
				bRet = false;
			}
			if(g_bHwEcc){
				if(!mtk_nand_check_dececc_done(j+1)){
					printk("error mtk_nand_check_dececc_done\n");
		            bRet = false;
	    		}
			}
			if(g_bHwEcc){     
				...
				{
					printk("error mtk_nand_check_bch_error\n");
					bRet = false;
				}
	        }
		...
		if (!mtk_nand_status_ready(STA_NAND_BUSY))
		{
			printk("error mtk_nand_status_ready\n");
			bRet = false;
		}
...
#if 0
	printk( "mtk_nand_exec_write_page, page: 0x%x\n", u4RowAddr);
#endif
#ifdef _MTK_NAND_DUMMY_DRIVER_
	...
	for (i=0;i<part_num;i++)
		...
		if ((page >= start_page) && (page <= end_page))
			...
			if (strncmp(g_pasStatic_Partition[i].name, "tp_data", sizeof("tp_data")) == 0
				|| strncmp(g_pasStatic_Partition[i].name, "data", sizeof("data")) == 0 || strncmp(g_pasStatic_Partition[i].name, 
				"radio", sizeof("radio")) == 0)
				{
				return 0;
				}
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(3,10,13)
int mtk_nand_read_oob(struct mtd_info *mtd, struct nand_chip *chip, int page)
#else
...
EXPORT_SYMBOL(mtk_nand_read_oob);
...
#endif
{
	...
	struct mtd_partition partitions[] = 
		{
			{.name = "null", .size = 0, .offset = 0},
		};
	struct mtd_info *pMtd = NULL;
...
/* move it ahead? */
	...
	/* parse uboot mtdparts */
	mtd_device_parse_register(mtd, NULL, NULL, partitions, ARRAY_SIZE(partitions));

	/* get the real partition table.*/
	/* TODO: need revision. */
	part_num = 0;
	mutex_lock(&mtd_table_mutex);

	mtd_for_each_device(pMtd) {
		/* whole flash is the last mtd */
		if (strncmp(pMtd->name, "wholeflash", sizeof("wholeflash")) == 0)
			break;
		
		g_pasStatic_Partition[part_num].size = pMtd->size;
		g_pasStatic_Partition[part_num].name = pMtd->name;
		g_pasStatic_Partition[part_num].offset =mtdpart_get_offset(pMtd);
		part_num ++;
	}
	mutex_unlock(&mtd_table_mutex);
	
	
	/* the last partition is whole flash. Only for upgrading, do not include it in partition table .*/
	//part_num --;
#endif
#endif
	printk("OK\n");
