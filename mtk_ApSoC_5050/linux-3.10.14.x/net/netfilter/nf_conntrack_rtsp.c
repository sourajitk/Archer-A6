                ...
                struct nf_conn *ct, enum ip_conntrack_info ctinfo)
{
	...
	while (dataoff < datalen) {
		...
		if (nf_nat_rtsp && ct->status & IPS_NAT_MASK)
			...
			ret = nf_nat_rtsp(skb, ctinfo, protoff, hdrsoff, hdrslen, &expinfo, exp, &sizeDiff);
...
static inline int
help_in(struct sk_buff *skb, unsigned int protoff, unsigned char *rb_ptr, unsigned int datalen,
 struct nf_conn* ct, enum ip_conntrack_info ctinfo)
{
	struct ip_ct_rtsp_expect expinfo;
...
static inline int
help_in_ex(struct sk_buff *skb, unsigned int protoff, unsigned char *rb_ptr, unsigned int datalen,
           struct nf_conn *ct, enum ip_conntrack_info ctinfo)
{
	char*   	pdata = rb_ptr;
	char*  		pTranData;
		...
		struct nf_conn *ct, enum ip_conntrack_info ctinfo) 
{
	...
	case IP_CT_DIR_REPLY:
		...
		/* inbound packet: server->client */
                ret = help_in(skb, protoff, rb_ptr, datalen, ct, ctinfo);
		if (NF_ACCEPT == ret)
		{
		    ret = help_in_ex(skb, protoff, rb_ptr, datalen, ct, ctinfo);
                }
		break;
	}
...
//		hlpr->name = &rtsp_names[i][0];
        sprintf(hlpr->name, &rtsp_names[i][0]);
