...
static int kill_all(struct nf_conn *i, void *data)
{
    ...
    if ((IPPROTO_TCP == protonum && dstport != htons(PPTP_DST_PORT)) ||
...
static int kill_dns(struct nf_conn *i, void *data)
{
    ...
    if ((IPPROTO_TCP == protonum && dstport == htons(DNS_DST_PORT)) ||
        (IPPROTO_UDP == protonum && dstport == htons(DNS_DST_PORT)))
        return 1;
    else
        return 0;
}
