...
#endif	
	if (br0_dev == NULL) {
		pr_info("br-lan_dev = NULL\n");
		return ;
	}
	br0_in_dev = in_dev_get(br0_dev);
	if (br0_in_dev == NULL) {
		pr_info("br-lan_in_dev = NULL\n");
		return ;
	}
...
#endif
	/* check skb->dev first to avoid panic problem when rmmod hw_nat.ko -- by wj 2019.8.1 */ 
...
int32_t PpeFillInL4Info(struct sk_buff * skb, struct FoeEntry * foe_entry, PktParseResult *PpeParseResult)
{
	...
	if (PpeParseResult->pkt_type == IPV4_HNAPT) {
		...
		if (PpeParseResult->iph.protocol == IPPROTO_TCP) {
			...
			/* add by wanghao  */
			if (foe_entry->ipv4_hnapt.new_dport == 0x35)//DNS port
			{
				return 1;
			}
			/* add end  */
...
#if 1//PCP(OSBN):keep ingress packet's 1st and 2nd VLAN Pbits and DEI
      iblk2->pcpl = 0;
#endif
}
#endif
...
/*TP PPE off, use ACL instead*/	
	...
	RegModifyBits(PPE_GLO_CFG, DFL_TTL0_DRP, 4, 0);
...
#endif
#else
		WanInt = ra_dev_get_by_name("eth1.1");
#endif
#endif
...
/*add by wanghao*/
static int read_proc(struct file *file, char __user *start, size_t count, loff_t *data)
{
	return count;
}

static int write_proc(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	char buf[20] = {0};
...
/*For FastPath disable crc check*/
	...
	/*add by wanghao*/
	if (!proc_create("tplink/hwnat", 0600, NULL, &proc_fops))
	{
		NAT_PRINT("create tplink/hwnat error...\n");
	}

	clearHnatCachePtr = clearHnatCache;
	/*add end*/
...
//    device_destroy(hnat_class, MKDEV(250, 0));
	...
	/*add by wanghao*/
	remove_proc_entry("tplink/hwnat", NULL);
	clearHnatCachePtr = NULL;
	/*add end*/
}
