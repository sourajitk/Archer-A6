...
void clearHnatCache(struct _ipv4_hnapt *ipv4_hnapt)
{
	int i = 0;
	struct FoeEntry *entry;
	/*printk("sp=%d dp=%d sip=%x dip=%x\n", 
		ntohs(ipv4_hnapt->sport), 
		ntohs(ipv4_hnapt->dport), 
		ntohl(ipv4_hnapt->sip), 
		ntohl(ipv4_hnapt->dip));*/
	
	for (i = 0; i < FOE_4TB_SIZ; i++)
	{
		entry = &PpeFoeBase[i];
		
		if ((ipv4_hnapt->sport == entry->ipv4_hnapt.sport 
			&& ipv4_hnapt->dport == entry->ipv4_hnapt.dport 
			&& ipv4_hnapt->sip == entry->ipv4_hnapt.sip 
			&& ipv4_hnapt->dip == entry->ipv4_hnapt.dip)
			|| 
			(ipv4_hnapt->sport == entry->ipv4_hnapt.new_dport
			&& ipv4_hnapt->dport == entry->ipv4_hnapt.new_sport
			&& ipv4_hnapt->sip == entry->ipv4_hnapt.new_dip
			&& ipv4_hnapt->dip == entry->ipv4_hnapt.new_sip))
		{
			entry->ipv4_hnapt.udib1.state = INVALID;
			entry->ipv4_hnapt.udib1.time_stamp = RegRead(FOE_TS) & 0xFF;
			...
			PpeSetCacheEbl(); /*clear HWNAT cache*/
		#endif
			//printk("foe unbind an entry!\n");
		}
	}
}

/* clear all HNAT entrys  */
void FoeUnbindAll(void)
{
	int i = 0;
	
	for (i = 0; i < FOE_4TB_SIZ; i++)
	{
		PpeFoeBase[i].ipv4_hnapt.udib1.state = INVALID;
		PpeFoeBase[i].ipv4_hnapt.udib1.time_stamp = RegRead(FOE_TS) & 0xFF;
	}
#if defined (CONFIG_HNAT_V2)
	PpeSetCacheEbl(); /*clear HWNAT cache*/
#endif
	//printk("foe unbind all entries!\n");
}
/*add end*/
