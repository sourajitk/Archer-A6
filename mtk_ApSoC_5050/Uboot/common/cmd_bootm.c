...
unsigned int load_uboot_data_addr = 0xa2000000;
...
image_header_t *fake_image_header(image_header_t *hdr, ulong kernelTextAddr, ulong entryPoint, int size)
{
	...
	hdr->ih_time  = 0;
	hdr->ih_size  = (size);
	hdr->ih_load  = ntohl(kernelTextAddr);
	hdr->ih_ep    = ntohl(entryPoint);
...
#if defined (CFG_ENV_IS_IN_NAND)
ulong find_header(ulong startaddr, ulong endaddr, ulong blocksize)
{
	image_header_t *hdr = &header;
	while (startaddr < endaddr)
	{
		if (startaddr >= CFG_FLASH_BASE)
			ranand_read(hdr, (char *)(startaddr - CFG_FLASH_BASE), sizeof(image_header_t));
		else
			memmove (hdr, (char *)startaddr, sizeof(image_header_t));
		...
		{
			printf ("find kernel header at 0x%x\n",startaddr);
			return startaddr;
		}
	}
...
ulong find_ubi(ulong startaddr, ulong endaddr, ulong blocksize)
{
	image_header_t hdr;
	while (startaddr < endaddr)
	{
		...
		if (memcmp((char *)&hdr, UBI_MAGIC, sizeof(UBI_MAGIC)-1) != 0)
		{
		...
		{
			printf ("find ubi at 0x%x\n",startaddr);
			return startaddr;
		}
	}

	printf ("can't find ubi header \n");
...
#endif
/* argv[0]: cmd name
	argv[1]: boot from addr
	argv[2]: load addr 
	argv[3]: boot type, kernel and so on
*/
extern 	struct mtk_nand_host *host;
extern	int g_next_boot_type;
int do_bootm (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
   ...
   if (addr == 0x8A200000)
	...
	if ((ntohl(hdr->ih_magic) != IH_MAGIC) 	)
		...
		SHOW_BOOT_PROGRESS (-1);
		return -ERR_BOOTM_MAGIC_DISMATCH;
	...
	/* check if boot type matches. */
	if  ((g_next_boot_type != IH_TYPE_INVALID) && (hdr->ih_type != g_next_boot_type))
		{
		
			printf("Not type %d at 0x%08X\n", g_next_boot_type, addr);
			SHOW_BOOT_PROGRESS (-1);
			return -ERR_BOOTM_TYPE_DISMATCH;
		}

	/* for firmware type, just return to tell the caller try the next header . */
	if (hdr->ih_type == IH_TYPE_FIRMWARE)
		{
		/* It's a firmware header, should jump to next one. */
			return -ERR_BOOTM_TRY_UBOOT;
		}
	...
	if (verify) {
		...
		unsigned long cvalue = crc32 (0, (char *)data, len);
		if (cvalue != ntohl(hdr->ih_dcrc)) {
			printf ("Bad Data CRC calc 0x%08x stored 0x%08x\n", cvalue, ntohl(hdr->ih_dcrc));
			SHOW_BOOT_PROGRESS (-3);
			return 1;
		}
		puts ("OK\n");
...
# error Unknown CPU type
	...
	case IH_TYPE_STANDALONE:
		...
		if (argc > 2) {
			ulong loadaddr=htonl(simple_strtoul(argv[2], NULL, 16));
			if (loadaddr != 0)
				hdr->ih_load = loadaddr;
		}
		break;
#endif
	case IH_TYPE_KERNEL:
	...
	case IH_TYPE_STANDALONE:
		...
		(*appl)(argc-1, &argv[1]);
#else
		/* should we clear all cache??? */
		//flush_cache(CFG_FLASH_BASE, gd->ram_size);
		void (*uboot_addr)(void);
		uboot_addr = appl;
		
		g_uboot_data.boot_address = addr;
		
		memmove((char *)load_uboot_data_addr, (char *)&g_uboot_data, sizeof(g_uboot_data));
...
#ifdef IH_TYPE_MULTI_SUPPORT
/* This will cost much time if the file is big. */
