...
extern uchar *env_get_addr_default(int);
...
/************************************************************************
************************************************************************/
static int envmatch_default (uchar *, int);
static int envmatch (uchar *s1, int i2);
static char *_getenv_default (uchar *name);

static char *_getenv (uchar *name);
/*
...
uchar env_get_char_default(int index)
{
	return default_environment[index];
}

static int _do_printenv(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	int i, j, k, nxt;
	int rcode = 0;
...
static int _do_printenv_default(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	int i, j, k, nxt;
	int rcode = 0;

	if (argc == 1) {		/* Print all env variables	*/
		for (i=0; env_get_char_default(i) != '\0'; i=nxt+1) {
			for (nxt=i; env_get_char_default(nxt) != '\0'; ++nxt)
				;
			for (k=i; k<nxt; ++k)
				putc(env_get_char_default(k));
			putc  ('\n');
	...
	for (i=1; i<argc; ++i) {	/* print single env variables	*/
		...
		for (j=0; env_get_char_default(j) != '\0'; j=nxt+1) {

			for (nxt=j; env_get_char_default(nxt) != '\0'; ++nxt)
				;
			k = envmatch_default(name, j);
			if (k < 0) {
				continue;
			}
			puts (name);
...
{
	int rcode = _do_printenv(cmdtp, flag, argc, argv);
#if defined(NO_SAVE_DEFAULT_ENV)
	printf("\nDefault Environment:\n");
	_do_printenv_default(cmdtp, flag, argc, argv);
#endif
	return rcode;
}
...
static int __do_setenv (int flag, int argc, char *argv[])
{
...
int _do_setenv(int flag, int argc, char*argv[])
{
	...
	char *def_val = _getenv_default(name);
	if (def_val && value && (strcmp(def_val, value) == 0))
		return 0;
#endif
	return __do_setenv(flag, argc, argv);
...
static char *_getenv_default (uchar *name)
{
	...
	for (i=0; env_get_char_default(i) != '\0'; i=nxt+1) {
		...
		if ((val=envmatch_default(name, i)) < 0)
			continue;
		return (env_get_addr_default(val));
	}
...
/* 	we do not save default env into flash. So,
	if can't get env in flash, try default. */
#if defined(TRY_DEFAULT_ENV)
	if (res == NULL)
		res = _getenv_default(name);
#endif
	return res;
}
 ...
 */
static int envmatch_default (uchar *s1, int i2)
{

	while (*s1 == env_get_char_default(i2++))
		if (*s1++ == '=')
			return(i2);
	if (*s1 == '\0' && env_get_char_default(i2-1) == '=')
		return(i2);
	return(-1);
}
