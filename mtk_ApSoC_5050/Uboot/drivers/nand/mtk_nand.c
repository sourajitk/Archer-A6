...
static int set_fact_bad(unsigned int page);
...
{
	/* mtd here is the master mtd. We are looking for the boundry of the mtd partition which the input block is located. */
	struct nand_chip *chip = mtd->priv;
	int i;
	...
	*start_blk = 0;	
        ...
        {
        /* we are sure that the last partition do not cover the last block, cause FACT_BBT is located there. So no need to check it here.
        By xjb, 2017.11.16*/
        #if 0
		if (i == (g_part_num  -1) )
		{
...
/* why??? Disable it. by xjb, 2017.11.16 */
#if 0
...
static int block_remap(struct mtd_info *mtd, int block)
{
	...
	/* look for a good block which is on the right side(or the same place) of block, and there are the same number of good blocks between them*/
	/* upper virtual block:1 2 3 4 5 6, flash block 1(OK) 2(bad) 3(OK) 4(bad) 5(OK) 5(OK) 6(OK) 
		now input block 3, it's map to block 5 in flash. 
		*/
	for (j = start_blk; j <= end_blk;j++)
	{
		/* check if this block is bad. 
			i.e. byte 10-11-11-00 represents block3(page1 page0)-block2(page1 page0)-block1(page1 page0)-block0(page1 page0) */
			/* j=0 1 2 3->(j<<1) & 0x6)->j=0 2 4 6 */
	...
	/* if we can find a good one in this mtd, use it */
	if (j <= end_blk)
	{
		return j;
	}
	else
	{
		...
		/* look for a bad block reversely for this input block. Leave enough bad blocks for the later input blocks. */
...
static int write_next_on_fail(struct mtd_info *mtd, char *write_buf, int page, int * to_blk)
{
	...
	buf = malloc(mtd->writesize + mtd->oobsize);
        ...
        {
		if (is_fact_bad((*to_blk) << page_per_block_bit) == 0)
		{
			int status;
			...
			if (status & NAND_STATUS_FAIL)
			{
				...
				set_fact_bad((*to_blk) << page_per_block_bit);
			}
			else
			{
				/* good block */
...
static int is_skip_bad_block(struct mtd_info *mtd, int page)
{
	...
	uint64_t end_page = 0;
	int i = 0;
	//printk("page_shift:0x%x, file system address:0x%x\n", chip->page_shift, FILE_SYSTEM_START_ADDRESS);
	/* DO NOT check jffs2 bad block. Jffs2 will handle them */

	for (i=0;i<g_part_num;i++)
		{
		start_page =  (g_pasStatic_Partition[i].offset >> chip->page_shift) ;
		end_page = start_page+ (g_pasStatic_Partition[i].size >> chip->page_shift) - 1;
		//printk("start_page 0x%lx, end_page 0x%lx, input page 0x%lx\n", (long unsigned)start_page,  (long unsigned)end_page,  (long unsigned)page);
		if ((page >= start_page) && (page <= end_page))
			{
			/* If the partition is filesystem, the check should be performed by the filesystem driver. Do not manage bad block here. */
			/* TODO: need revision */
			/*
			if (strncmp(g_pasStatic_Partition[i].name, "tp_data", sizeof("tp_data")) == 0)
				{
				return 0;
				}
			*/
			/* For readonly partitions, it's OK to use skip-bad-block mechanism. */
#if 0
			if (strncmp(g_pasStatic_Partition[i].name, "rootfs", sizeof("rootfs")) == 0)
				{
				return 0;
				}
#endif
	...
	/* if no partitions, do not enable skip_bad_block. */
	return 0;
}
...
static int mtk_nand_write_page(struct mtd_info *mtd, struct nand_chip *chip, const u8 * buf, int page, int cached, int raw)
{
	...
	if (shift_on_bbt)
	{
		...
		if (mapped_block == -1)
			return NAND_STATUS_FAIL;
/* block_remap has checked the fact bbt. */
#if 1
		if (is_fact_bad(mapped_block << (chip->phys_erase_shift - chip->page_shift)) != 0x0)
			return NAND_STATUS_FAIL;
#endif
	}
    }
	...
	else
	{
		...
		set_fact_bad(page_in_block + mapped_block * page_per_block);
...
static int mtk_nand_read_page(struct mtd_info *mtd, struct nand_chip *chip, u8 * buf, int page)
{
	...
	if (shift_on_bbt)
	{
		...
		if (mapped_block == -1)
			return NAND_STATUS_FAIL;
/* block_remap has checked fact bbt */
#if 1
		if (is_fact_bad(mapped_block << (chip->phys_erase_shift - chip->page_shift)) != 0x0)
			return NAND_STATUS_FAIL;
#endif
...
int mtk_nand_block_eraseoob_hw(struct mtd_info *mtd, loff_t offset)
{
    struct nand_chip *chip = mtd->priv;
    int block = (int)offset >> chip->phys_erase_shift;
    int page = block * (1 << (chip->phys_erase_shift - chip->page_shift));
    ...
    ret = mtk_nand_write_oob_raw(mtd, buf, page, sizeof(buf));
    return ret;
}
...
#endif
	//printk("%s read page 0x%x's oob\n",  mtd->name, page_in_block + mapped_block * page_per_block);
...
static int set_fact_bad(unsigned int page)
{
	...
	chip  = &host->nand_chip;
	block = page >> (chip->phys_erase_shift - chip->page_shift);
	fact_bbt[block  >> 2] |= 0x3 << ((block*2) & 6);
	return 0;
}
...
int load_fact_bbt()
{
	...
	for (i = total_block - 1; i >= (total_block - FACT_BBT_BLOCK_NUM); i--)
	{
		if (read_fact_bbt(i << (chip->phys_erase_shift - chip->page_shift)) == 0)
		{
			...
			//return 0;
			/* DO NO return here. We still need to scan the whole flash to get all bad blocks. */
			have_fact_bbt = true;
			break;
		}
	}
	...
	/* DETAIL: This process scans whole NAND FLASH to build FACTORY BAD BLOCK table.
	After programming, there are MTK layout blocks and original layout blocks as well on the FLASH. 
	The original layout's bad block sign position is not the same as that of MTK layout, so bbt is used. 
	If factbbt is found, we still need to scan the whole flash to find those bad block mark bad in MTK format. 
	*/
	for (i = 0; i < total_block; i++)
	{
		...
		for (page = i << (chip->phys_erase_shift - chip->page_shift); page <= ((i << (chip->phys_erase_shift - chip->page_shift)) + 1) ; page++)
		{
			...
			else
			{
				if (mtk_nand_read_oob_hw(mtd, chip, page)==0)
				{
					/* if no fact bbt, this is a fresh FLASH. All bad blocks should be in the normal form. */
					if (!have_fact_bbt)
					{
						if (chip->buffers->databuf[fact_bad_pos] != 0xff)
						{
							...
							{
								fact_bbt[i >> 2] |= 0x3 << ((i*2) & 6);
								printf("detect factory bad block at 0x%x (%d)\n", (unsigned int) i, (unsigned int)(page & 0x1)+1);
								continue;
							}
						}
					}
					else
					/* we have fact bbt. Only check if oob[0] is 0xff or not. */
					{
						if (chip->oob_poi[0] != 0xff)
						{
							fact_bbt[i >> 2] |= 0x3 << ((i*2) & 6);
							printf("detect user-defined bad block at 0x%x (%d)\n", (unsigned int) i, (unsigned int)(page & 0x1)+1);
							continue;
						}
...
#if defined (__KERNEL_NAND__)	
	err = add_mtd_partitions(mtd, g_exist_Partition, g_part_num);	
	//err = mtd_device_register(mtd, g_exist_Partition, g_part_num);
#endif	
#else
//#if defined (__KERNEL_NAND__)
	//err = mtd_device_register(mtd, g_pasStatic_Partition, g_part_num);
	//err = add_mtd_partitions(mtd, g_pasStatic_Partition, g_part_num);
	/* we do not know the partition table. Just assum that it has only one partition. */
	/* partition table should be init after env is read out! */
	//init_partable(nand_chip);
...
uint64_t get_nand_chip_size()
{
	return host->nand_chip.chipsize;
}



void add_partition(struct nand_chip *nand, const char *name, int offset, int size)
{
	strncpy(g_pasStatic_Partition[g_part_num].name, name, sizeof(g_pasStatic_Partition[g_part_num].name));
	
	if (offset > nand->chipsize)
		offset = nand->chipsize;
	g_pasStatic_Partition[g_part_num].offset = offset;
	
	if (size + offset > nand->chipsize)
		size = nand->chipsize - offset;
	g_pasStatic_Partition[g_part_num].size = size;

	if (g_part_num > 0 && 
		(g_pasStatic_Partition[g_part_num-1].size + g_pasStatic_Partition[g_part_num-1].offset > g_pasStatic_Partition[g_part_num].offset)
		)
		{
		g_pasStatic_Partition[g_part_num-1].size = g_pasStatic_Partition[g_part_num].offset - g_pasStatic_Partition[g_part_num-1].offset;
		}
	
	g_part_num ++;
}

int parse_mtdparts(mtd_partition *part, int part_len)
{
	int part_count = 0;
	...
	if (str)
		{
		str = strstr(str, "mtdparts=");
		if (str)
			{
			char *temp = NULL;
			int name_len = 0;
			while(*str != ' ' && part_count < part_len)
				{
				if (*str == '@')
					{
					part[part_count].size = simple_strtoul(str-sizeof("0x00240000") + 1, NULL, 16);
					part[part_count].offset = simple_strtoul(str + sizeof("@") -1, NULL, 16);
					str = strstr(str, "(");
					temp = strstr(str,")");
					if (str && temp)
						...
						{
						name_len = temp - str - 1;
						if (name_len > sizeof(part[part_count].name) - 1)
							name_len = sizeof(part[part_count].name)  -1;
						
						strncpy(part[part_count].name, str +1 ,name_len);

						/* stop at wholeflash. wholeflash is a fake mtd for dynamic changing partition table.*/
						if (strcmp(part[part_count].name, "wholeflash") == 0)
							break;
...
mtd_partition *get_part(const char *name)
{	
	int i = 0;
	for (i=0;i<g_part_num; i++)
		{
		if (strcmp(g_pasStatic_Partition[i].name, name) == 0)
			return &g_pasStatic_Partition[i];
		}
...
/* 
Detail:
The partition table is stored in ubootenv partition as bootargs.
For the firstuboot, no need to know the ubootenv;
For the mainuboot, should know the address of ubootenv and read bootargs from it.
For the imageuboot, if ubootenv is null, it should save its ubootenv into the ubootenv partition. 
*/
void init_partable(void)
{
	struct nand_chip *nand = &host->nand_chip;
	/* parse bootargs in ubootenv . 
	The firstuboot will return 0;
	The mainuboot will return the whole partiton table if the ubootenv is correct; Or return 0.
	Under such conditions, we should make up some virtual partitions for them. 
	*/
	g_part_num = parse_mtdparts(g_pasStatic_Partition, MAX_PARTITIONS_NUM);

	if ( g_part_num <= 0)
		{
		add_partition(nand, "part0", 0, nand->chipsize);
		int env_offset = 0;
		int next_addr = CFG_NEXT_BOOT_OFFSET;
#ifdef USE_UBOOT_ENV
		env_offset = CFG_ENV_ADDR -CFG_FLASH_BASE;
#endif
		/* if ubootenv address is specified, we can make up a ubootenv partition. */
		if (env_offset)
			{
			if (env_offset > next_addr)
				{
				add_partition(nand, "part1", next_addr, nand->chipsize);
				add_partition(nand, "part2", env_offset, nand->chipsize);
				}
			else
				{
				add_partition(nand, "part1", env_offset, nand->chipsize);
				add_partition(nand, "part2", next_addr, nand->chipsize);
...
#if 0
	add_partition(nand, "ubootfactory", 0, nand->chipsize);
	
#ifdef USE_UBOOT_ENV
	add_partition(nand, "ubootenv", CFG_ENV_ADDR -CFG_FLASH_BASE , nand->chipsize);

#endif

#if defined(BOOT_FROM_IMAGE)
	char *str = getenv(BOOT_OS0_ADDR);
	if (str)
		{
		add_partition(nand, "os0", simple_strtoul(str, NULL, 16), nand->chipsize);
		
		str = getenv(BOOT_OS1_ADDR);
		if (str)
			{
			add_partition(nand, "os1", simple_strtoul(str, NULL, 16), nand->chipsize);
			}
		}
	else
		{
		add_partition(nand, "os0", CFG_NEXT_BOOT_OFFSET, nand->chipsize);
		}
#endif

#if defined(DUAL_IMAGE_SUPPORT)
	add_partition(nand, "os0", CFG_IMAGE0_ADDR-CFG_FLASH_BASE,  CFG_IMAGE0_SIZE);
		
	add_partition(nand, "os1", CFG_IMAGE1_ADDR-CFG_FLASH_BASE,  CFG_IMAGE0_SIZE);
#endif
#endif
...
/* while (datalen || ooblen) {*/
	while (datalen) {
		...
		/* we use skip block. mtk_nand_read_page will handle it */
#if 0
#ifdef FACT_BBT
...
#define BLOCK_ALIGNED(a) ((a) & (mtd->erasesize - 1))
	...
	while (len) {
		...
		/* we use skip block. mtk_nand_erase will handle it. */
#if 0
#ifdef FACT_BBT
		if (is_fact_bad(page)) {
...
/* we use skip block. mtk_nand_erase will handle it. */
		...
		if (status & NAND_STATUS_FAIL) {
			...
			//return -1;
			/* let the erase process go ahead, because we may want to erase a whole partition. */
			ret = -1;
		}
		printf(".");
...
#endif
		/* select device and check wp */
		/* for erase raw, do not use skip bad block. */
		/* erase oob first */
		mtk_nand_block_eraseoob_hw(mtd, offs);
#if defined(SKIP_BAD_BLOCK)
		int shift_on_bbt_bak = 0;
		shift_on_bbt_bak = shift_on_bbt;
		shift_on_bbt = 0;
#endif
		status = mtk_nand_erase(mtd, page);
#if defined(SKIP_BAD_BLOCK)
		shift_on_bbt = shift_on_bbt_bak;
#endif
		/* See if block erase succeeded */
		if (status & NAND_STATUS_FAIL) {
			printf("%s: failed erase, page 0x%08x status=%08X\n", __func__, page, status);
...
int ranand_write_with_mtk_oob(char *buf, unsigned int to, int datalen)
{
	int page, i = 0;
	size_t retlen = 0;
...
/* mtk_nand_write_page will handle it */
#if 0	
#ifdef FACT_BBT
		if (is_fact_bad(page)) {
			addr += mtd->erasesize;
		...
		if (buf && len > 0) {
			for (i=0;i*NAND_SECTOR_SIZE<mtd->writesize;i++)
				{
				/* chip interface: 2048(data)+64(oob)
				input file : 512+16 512+16 512+16 512+16
				chip internal :512+16 512+16 512+16 512+16
				*/
					memcpy(buffers + offs + i* NAND_SECTOR_SIZE, 
						buf + i*(NAND_SECTOR_SIZE+OOB_PER_SECTOR), NAND_SECTOR_SIZE);	// we can not sure ops->buf wether is DMA-able.
				}

			/* no need to get the oob data. */
#if 0

			/* get oob data from input file*/
			/* chip interface: 2048(data)+64(oob)
				input file : 512+16 512+16 512+16 512+16
				chip internal :512+16 512+16 512+16 512+16
				we get 2048+64 from the file
			*/
			memset(oob_poi,0,sizeof(oob_poi));

			for (i = 0; i * (OOB_PER_SECTOR) < mtd->oobsize; i++)
			{
			memcpy(oob_poi+i * (OOB_AVAI_PER_SECTOR) , 
				buf+i *((NAND_SECTOR_SIZE)+(OOB_PER_SECTOR)) +(NAND_SECTOR_SIZE) , 
				(OOB_AVAI_PER_SECTOR));

			memcpy(oob_poi+mtd->oobsize /(OOB_PER_SECTOR)*OOB_AVAI_PER_SECTOR +  i*(OOB_PER_SECTOR-OOB_AVAI_PER_SECTOR), 
				buf+i *((NAND_SECTOR_SIZE)+(OOB_PER_SECTOR)) +(NAND_SECTOR_SIZE) + OOB_AVAI_PER_SECTOR, 
				(OOB_PER_SECTOR-OOB_AVAI_PER_SECTOR));
			}
#endif
			/* NOTICE: oobsize */
			buf += (len + mtd->oobsize);
			datalen -= (len + mtd->oobsize);
			retlen += (len + mtd->oobsize);
		}
		...
		/* FIXME: the oob data can't be written into flash correctly.
		Perhaps the chip modifies some oob data.
		So do not write the oob data here.
		*/
#if 0
		/* write oob */
		/* update nand_chip->oob_poi */
		mtk_nand_read_oob_hw(mtd, nand_chip, page);	

		memcpy(nand_chip->oob_poi, oob_poi, mtd->oobsize);
		
		mtk_nand_write_oob_hw(mtd, nand_chip, page);

		//mtk_nand_write_oob_raw(mtd, oob_poi, page, mtd->oobsize);
		if (page == 0)
			{
			int i =0;
			for (i = 0; i< mtd->oobsize;i++)
				{
				printf("%02x ", oob_poi[i]);
				if (i%16==15)
					printf("\n");
				}
			printf("\n");
...
int g_flash_table_file_len[MAX_PARTITIONS_NUM];
int ranand_flash_table(char *buf, int len)
{
	int i = 0;
	struct nand_chip *nand_chip;
  
	nand_chip  = &host->nand_chip;
	mtd_partition whole_flash_par;

	/* backup the last partition. */
	whole_flash_par = g_pasStatic_Partition[g_part_num];
	
	memset(g_pasStatic_Partition, 0, MAX_PARTITIONS_NUM *sizeof(mtd_partition));
	memset(g_flash_table_file_len, 0, sizeof(g_flash_table_file_len));
	g_part_num= 0;

	printf("Warning: partition table changed!\n");
	struct mtd_info *mtd = &host->mtd;
	for (i=0;*(uint32_t*)(buf)!=0xffffffff && i*16<len; i+=1, buf+=16)
		{
		/* the number in the file is little-endian! 
		0~31: partition start block addr
		32~63: partition end block addr
		64~95: data size(block)
		96~127:ffffffff
		*/
		g_pasStatic_Partition[i].offset = ((__le32_to_cpu(*(uint32_t*)buf)) << nand_chip->phys_erase_shift);
		g_pasStatic_Partition[i].size = ((__le32_to_cpu(*(uint32_t*)(buf+4))+1)<< nand_chip->phys_erase_shift) - g_pasStatic_Partition[i].offset;
        /* including spare data */
		g_flash_table_file_len[i] = (((__le32_to_cpu(*(int32_t*)(buf+8))<<nand_chip->phys_erase_shift))/mtd->writesize)*(mtd->writesize + mtd->oobsize);
		printf("*****************************************************************\n");
		printf("flash table:\n");
		printf("partition %d: offset 0x%x, size 0x%x, filelen 0x%x\n", 
			i, g_pasStatic_Partition[i].offset, g_pasStatic_Partition[i].size, g_flash_table_file_len[i]);
		printf("*****************************************************************\n");
		}
	g_part_num = i;
	g_pasStatic_Partition[g_part_num] = whole_flash_par;

	return 0;
}

int ranand_flash_file(char *buf, int len)
{
	int i = 0;
	int file_offset = 0;
	struct mtd_info *mtd = &host->mtd;
	for (i=0;i<g_part_num && file_offset<len;i++)
		{
		file_offset = (g_pasStatic_Partition[i].offset / mtd->writesize)*(mtd->writesize + mtd->oobsize);
		printf("writing partition %d...\n",i);
		printf("data len 0x%x, data offset 0x%x, flash offset 0x%x\n", g_flash_table_file_len[i], file_offset, g_pasStatic_Partition[i].offset);
		ranand_write_with_mtk_oob(buf+file_offset, g_pasStatic_Partition[i].offset, g_flash_table_file_len[i]);
		printf("write partition %d done\n",i);
		}
...
int ranand_erase_write(char *buf, unsigned int offs, int count)
{
	...
	int blink_count = 0;
	/* we do not use MTK partition. by xjb */
#if 0
...
#else
	if ((uint64_t)count > (nand_chip->chipsize )) 	{
		printf("Abort: image size larger than %lld!\n\n", nand_chip->chipsize  
					);
		return -1;
	}
...
#define BLOCK_ALIGNED(a) ((a) & (blocksize - 1))
		...
		if (BLOCK_ALIGNED(offs) || (count < blocksize))
		{
			...
			if (!block_orig) {
				printf("%s: malloc block failed,blocksize=%d,addr=0x%x\n", __func__,blocksize, &mtd->erasesize);
				return -1;
			}
...
try_next_1:
			...
			printf(".");
			#define NAND_ERASE_BLINK_PERIOD 10

			power_led_blink_ratelimit(&blink_count, NAND_ERASE_BLINK_PERIOD);
			buf += aligned_size;
			offs += aligned_size;
			count -= aligned_size;
		}
...
int ralink_nand_command(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	if (!strncmp(argv[1], "id", 3)) {
		...
		mtk_nand_read_oob_hw(mtd,nand_chip, page_addr);
		
		for (i = 0; i < mtd->oobsize; i++)
			printf("%02x%c",  nand_chip->oob_poi[i], (i%spare_per_sector == spare_per_sector-1)? '\n':' ');
		printf("\n");
	}
	else if (!strncmp(argv[1], "cp.o", 5))
		{
		unsigned int addr, l;
			...
			ranand_erase(addr, l );
			printf("write offs 0x%x, len 0x%x\n", addr, l);
			ranand_write_with_mtk_oob(p, addr, l);
		}
	else if (!strncmp(argv[1], "cp.b", 5)) {
			unsigned int addr, l;
		...
		for (i = 0; i < mtd->oobsize; i++)
			printf("%02x%c", nand_chip->oob_poi[i], (i%32 == 31)? '\n':' ');
		mtk_nand_write_oob_hw(mtd, nand_chip, page_addr);	
	}
	...
	else if (!strncmp(argv[1], "flashtable", sizeof("flashtable"))){
		addr = (unsigned int)simple_strtol(argv[2], NULL, 16);
		len = (int)simple_strtoul(argv[3], NULL, 16);
		printf("upload flash table file in addr 0x%x\n", addr);
		ranand_flash_table(addr, len);
	}
	else if (!strncmp(argv[1], "flashfile", sizeof("flashfile"))){
		addr = (unsigned int)simple_strtol(argv[2], NULL, 16);
		len = (int)simple_strtoul(argv[3], NULL, 16);
		printf("upload flash file in addr 0x%x\n", addr);
		
		printf("erase whole flash\n");
		ranand_erase(0, nand_chip->chipsize);
		printf("erase whole flash done\n");
		
		printf("write the whole flash\n");
		ranand_flash_file(addr, len);
		printf('write the whole flash done\n');
	}
	else
...
U_BOOT_CMD(
	...
	"nand usage:\n"
	"  nand id\n"
	"  nand read <addr> <len> using skip-bad-block\n"
	"  nand write <addr> <data...>  using skip-bad-block\n"
	"  nand cp.b <data addr> <flash addr> <len>  using skip-bad-block\n"
	"  nand cp.o <data addr> <flash addr> <len>  using skip-bad-block and oob\n"
	"  nand page <number> ignore skip-bad-block\n"
	"  nand erase <addr> <len>  using skip-bad-block\n"
	"  nand oob <page addr> ignore skip-bad-block\n"
	"  nand woob <page addr> <offset> <data> ignore skip-bad-block\n"
	"  nand marb <addr> ignore skip-bad-block\n"
	"  nand rawe <addr> <len>: erase all including oob and ignore skip-bad-block\n"
	"  nand dump\n"
	"  nand init\n"
	"  nand flashtable <addr> <len>: load flash partition table from mem\n"
	"  nand flashfile <addr> <len>: write flash file from mem into flash according to flashtable. It will erase the whole flash first."
);
